...
export function ContentCard({ item, aspectRatio = 'video', className = 'flex-shrink-0 w-48 md:w-64 snap-start' }: { item: ContentItem; aspectRatio?: 'video' | 'square' | 'portrait', className?: string }) {
  ...
  return (
    <Link href={item.url} className={`group block relative ${className}`}>
      <div className={`relative overflow-hidden bg-card-bg ${aspectClasses[aspectRatio]}`}>
        ...
        {item.audioUrl && (
          <button 
            onClick={handlePlay}
            className="absolute bottom-3 right-3 w-10 h-10 bg-accent text-white rounded-full flex items-center justify-center translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 shadow-lg"
          >
